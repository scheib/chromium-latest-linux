...
if [ -e "State/UPDATING" ]; then
    notify-send -t 20000 -i "$HOME/.Chromium/Chromium Update.png" "Updating Chromium..." "Chromium is currently being updated ..."
    exit 0
fi
...
#    echo "already have latest version"
    notify-send -t 8000 -i "$HOME/.Chromium/Chromium Updated.png" "Updated (Rev.: $REVISION)" "Chromium is in its Latest Version"
  exit
fi
...
rm -rf "$OLD_VERSION"
notify-send -t 20000 -i "$HOME/.Chromium/Chromium Updated.png" "Chromium Updated" "SChromium has been Successfully Updated. Chromium is in its Latest Version ($REVISION)"

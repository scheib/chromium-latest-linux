...
if [ $KernelOS == "Darwin" ]; then rm -rf /Applications/Chromium.app && ln -s $(pwd)/latest/Chromium.app/ /Applications/Chromium.app && osascript -e 'display notification "Chromium has been Successfully Updated. Chromium is in its Latest Version ($REVISION)" with title "Chromium Updated" sound name "Submarine"' ; fi
